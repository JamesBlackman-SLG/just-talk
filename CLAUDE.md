...
Voice-to-text tool for Hyprland/Wayland. Hold Right Alt (AltGr) to record speech, which gets transcribed and typed into the focused window.

## Architecture

Single-binary Rust app. Tokio async runtime for the main event loop; blocking threads for evdev input and Wayland overlay.
...
Double-tap AltGr to toggle blips on/off at runtime. Plays a two-note confirmation tone (ascending C5→G5 for ON, descending G5→C5 for OFF). State persists to config file.
